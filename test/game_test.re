...
let player =
  Alcotest.testable(
    Fmt.using(Tennis.Game.string_of_player, Fmt.string),
    (==),
  );

let score =
  Alcotest.testable(
    Fmt.using(Tennis.Game.string_of_score, Fmt.string),
    (==),
  );
...
let given_advantage_when_advantaged_player_wins = () => {
  let advantaged_player = Player_one;
  let winner = advantaged_player;
  Alcotest.(check(score))(
...
let transitions_set =
  Alcotest.[
    ...
    test_case(
      "Given Advantage when advantaged player wins then score is Game advantaged_player",
      `Quick,
    ...
    test_case(
      "Given Player_one at 40 when Player_one wins then score is Game Player_one",
      `Quick,
    ...
    test_case(
      "Given Player_one at 40 | other at 0 when other wins then score is 40 / 30",
      `Quick,
      given_player_one_at_40_other_at_15_when_other_wins,
    ),
    test_case(
      "Given Player_one at 15 | other at 15 when other wins then score is 30 / 15",
      `Quick,
    ...
    test_case(
      "Given Player_one at 30 | other at 15 when other wins then score is 40 / 15",
      `Quick,
